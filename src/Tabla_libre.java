...
public class Tabla_libre extends JPanel implements Serializable{
...
public Tabla_libre(int c, int f, JFrame v , int r,int b,int l){
    super();
    base=b;
    record=r;
    juego=v;
    ...
    limite=l;
    //columnas=c;
    this.tamanioMatriz = new int[filas][c];
    this.setLayout(new GridBagLayout());
    this.numeros= new JButton[filas][c];
    this.botones= new JButton[c];
    columnas=c;
    aux=new Opciones(1,record,filas, juego,base,limite);
    inicializaComponentes();
        aux.setVisible(true);
        aux.cambiarTitulo(c,filas);
        aux.cambiarPuntuacion(0);
}

private int generar(int n){
    int potencia = (int) (Math.random() * 6) + 1;
    int aux2=(int) Math.pow(2, potencia);
...
botones[(tamanioMatriz[0].length-1)/2].setFont(new Font("Arial", Font.BOLD, 30));
...
if(aux.getPuntucion() >= record){
    System.out.println("Has ganado");
    juego.setVisible(false);
}
else{
    game_over fin=new game_over(columnas,filas,record,1,base,limite);
    fin.setVisible(true);
    juego.setVisible(false);
...
aux.cambiarPuntuacion(tamanioMatriz[f][c]);
for (int i=f; i< tamanioMatriz.length-1; i++){
tamanioMatriz[i][c-1]= tamanioMatriz[i+1][c-1];
tamanioMatriz[i][c+1]= tamanioMatriz[i+1][c+1];
}
